...
bool load_mono_float_wav(const std::string& path, WavLoadResult& out, std::string& error) {
    ...
    if (audio_format != kWaveFormatIeeeFloat) {
        error = "Not 32-bit float PCM: audio format is " + std::to_string(audio_format) +
                " (expected " + std::to_string(kWaveFormatIeeeFloat) + " IEEE float).";
        return false;
    }
    if (bits_per_sample != 32) {
        error = "Not 32-bit float PCM: bits per sample is " + std::to_string(bits_per_sample) +
                " (expected 32).";
        return false;
    }
    if (num_channels != 1) {
        error = "Not mono: channel count is " + std::to_string(num_channels) + " (expected 1).";
        return false;
    }
    ...
    for (uint32_t i = 0; i < total_samples; ++i) {
        out.samples[i] = f32_le_from_bytes(pcm_bytes.data() + i * 4);
    }
...
int main() {
    const std::string path = "input.wav";
